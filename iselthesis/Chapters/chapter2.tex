...
\begin{table}[H]
    ...
    \begin{tabular}{cccc}
        \toprule
        Vértice & Curso & Disciplina & Período \\ \midrule
        $m_{11}$  & $K_{1}$ & $l_{1}$      & 1       \\
        $m_{21}$  & $K_{2}$ & $l_{1}$      & 2       \\
        $m_{22}$  & $K_{2}$ & $l_{2}$      & 3       \\
        $m_{31}$  & $K_{3}$ & $l_{1}$      & 1       \\
...
\begin{table}[H]
    \centering
    \caption{Categorias de instâncias}
    \label{tabela:categorias-instancias}
    \begin{tabular}{ccc}
    \toprule
    Instâncias & Eventos     & Salas   \\ \midrule
...
% TODO: Repete parte da secção 2.2.2
...
Rede de fluxo (\textit{Network Flow})~\cite{LinearProgrammingNetwork-Bazaraa-2009} é um conceito da teoria dos grafos. O fluxo é representado como um grafo direcionado, ou seja, segue uma direção de acordo com as setas representadas nas arestas. Existem duas regras para o funcionamento correto do fluxo. O fluxo que entra num nó deve ser igual ao fluxo que sai do nó. Cada aresta possui uma capacidade máxima de fluxo que deve ser respeitada. Este algoritmo normalmente é utilizado para problemas como logística e transporte, redes de comunicação e informações, energia e distribuição de recursos, entre outros.
...
Os critérios de paragem mencionados podem incluir um número máximo de iterações, um tempo limite de execução ou a obtenção de uma solução suficientemente boa, dependendo do problema e dos requisitos de desempenho.

Os critérios de aspiração são regras especiais que permitem movimentos que estão presentes na lista tabu. Um exemplo de um critério de aspiração é aceitar movimentos proibidos se resultarem em uma solução significativamente melhor do que a melhor solução encontrada até o momento.
