...
Um problema \gls{np} é um problema cuja solução pode ser verificada em tempo polinomial por uma máquina de Turing não determinística \cite{}. Um problema é considerado NP-completo se sua solução puder ser verificada em tempo polinomial por uma máquina de Turing não determinística e se qualquer problema da classe \gls{np} puder ser reduzido a ele em tempo polinomial.
...
Ao modelo matemático apresentado foi acrescentada a seguinte função objetivo, para otimizar as soluções obtidas, maximizando ou minimizando o custo das soluções.

\begin{equation}
    \text{max } \displaystyle\sum_{i=1}^{q} \displaystyle\sum_{k=1}^{p} C_{ik}y_{ik},
\end{equation}

em que $C_{ik}$ - Medida de desejabilidade de ter uma aula do curso $K_{i}$ no período k.

% Dúvida, confirmar com  o professor
...
\section{\textit{International Timetabling Competition} 2019}
...
% TODO: Repetir parte da secção 2.2.2
...
Rede de fluxo (\textit{Network Flow})~\cite{} é um conceito da teoria dos grafos. O fluxo é representado como um grafo direcionado, ou seja, segue uma direção de acordo com as setas representadas nas arestas. Existem duas regras para o funcionamento correto do fluxo. O fluxo que entra num nó deve ser igual ao fluxo que sai do nó. Cada aresta possui uma capacidade máxima de fluxo que deve ser respeitada. Este algoritmo normalmente é utilizado para problemas como logística e transporte, redes de comunicação e informações, energia e distribuição de recursos, entre outros.
...
Procura Tabu (\textit{Tabu Search})~\cite{MetaheuristicsDesignImplementation-Talbi-2009} é uma meta-heurística e um procedimento adaptativo que guia algoritmos de pesquisa local na exploração de espaços de busca. Esta técnica é tipicamente determinística (depende da implementação), costuma ser utilizada para resolver problemas complexos e tem como objetivo evitar que a busca fique presa em mínimos ou máximos locais.
...
O conceito de trocas de cadeia de Kempe (\textit{Kempe chain interchange}), permite a criação de grafos vizinhos através da troca das cores das cadeias. Na Figura~\ref{fig:exemplo-kempe-chain-neighbourhood-1} é apresentado um exemplo com variantes do mesmo grafo criados através das trocas de cores mencionadas.

\begin{figure}[ht]
    \centering
    \includegraphics[width=.85\linewidth]{exemplo-kempe-chain-neighbourhood-1}
    \caption{Exemplo da troca de cores nas cadeias de Kempe (adaptado de:\cite{GuideGraphColouring-Lewis-2016})}
...
Na Figura~\ref{fig:exemplo-kempe-chain-neighbourhood-2} (c), introduz-se o conceito de troca de pares (\textit{pair swap}). Neste caso, ocorre a troca de cores nas cadeias KEMPE($v1$, 3, 4) e KEMPE($v5$ , 4, 3), as quais possuem apenas um vértice. A troca de pares resulta numa nova distribuição de cores que continua a satisfazer as restrições do problema.