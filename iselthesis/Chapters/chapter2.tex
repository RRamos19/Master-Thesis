...
Um problema \gls{np} é um problema cuja solução pode ser verificada em tempo polinomial por uma máquina de Turing não determinística. Um problema é considerado NP-completo se sua solução puder ser verificada em tempo polinomial por uma máquina de Turing não determinística e se qualquer problema da classe \gls{np} puder ser reduzido a ele em tempo polinomial.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.6\linewidth]{grafo-complexo-dewerra}
...
No artigo~\cite{HybridSimulatedAnnealing-Tuga-2007} é apresentada uma solução híbrida com a utilização do \gls{sa}. Nesta solução, para além do uso do \gls{sa}, foi utilizado um algoritmo heurístico baseado em grafos para criar uma solução inicial. Através de uma heurística baseada no \textit{Kempe Chain Neighbourhood} (descrito na Secção~\ref{explic:kempe-chain-neighbourhood}), denominada de \textit{Pair-wise Kempe Chain}, incorporada no \gls{sa} é possível minimizar o número de restrições flexíveis violadas mas também melhorar as trocas de eventos em caso de conflitos sem violar as restrições rígidas.
...
No artigo~\cite{Featurebasedtuning-Bellio-2016} é utilizado o algoritmo \gls{sa} para a resolução do problema combinatório.
...
Procura Tabu (\textit{Tabu Search})~\cite{MetaheuristicsDesignImplementation-Talbi-2009} é uma meta-heurística e um procedimento adaptativo que guia algoritmos de pesquisa local na exploração de espaços de busca. O \textit{Tabu Search} é um algoritmo tipicamente determinístico (depende da implementação) e costuma ser utilizado para resolver problemas complexos e tem como objetivo evitar que a busca fique presa em mínimos ou máximos locais.
...
Através da utilização da \textit{Tabu list} são memorizados locais explorados recentemente com o objetivo de evitar ciclos e revisitas. Embora movimentos tabu sejam proibidos, pode haver exceções se eles resultarem numa solução significativamente melhor, sendo controlado por critérios de aspiração. Alguns mecanismo complexos incluem a utilização de memórias de meio termo (Intensificação) e longo termo (Diversificação).

Em termos gerais, uma versão simplificada do \textit{Tabu Search}, com o objetivo de minimizar o custo, opera da seguinte forma:
...
\begin{compactitem}
    ...
    \item É repetido o algoritmo até que o critério de paragem seja satisfeito. O resultado é a melhor solução encontrada até ao momento.
\end{compactitem}
...
Em termos gerais, o \gls{sa}, com o objetivo de minimizar o custo, opera da seguinte forma:
...
Na Figura~\ref{fig:exemplo-kempe-chain-neighbourhood-1} é apresentado um exemplo com variantes do mesmo grafo criados através das trocas de cores mencionadas.

\begin{figure}[ht]
    \centering
    \includegraphics[width=.85\linewidth]{exemplo-kempe-chain-neighbourhood-1}
    \caption{Exemplo da troca de cores nas cadeias de Kempe (Fonte:\cite{GuideGraphColouring-Lewis-2016})}
...
\begin{figure}[ht]
    ...
    \caption{Exemplo da troca de pares nas cadeias de Kempe (Fonte:\cite{GuideGraphColouring-Lewis-2016})}
    \label{fig:exemplo-kempe-chain-neighbourhood-2}
\end{figure}

Na Figura~\ref{fig:exemplo-kempe-chain-neighbourhood-2} (c), introduz-se o conceito de troca de pares (\textit{pair swap}). Neste caso, ocorre a troca de cores nas cadeias KEMPE($v1$, 3, 4) e KEMPE($v5$ , 4, 3), cadeias as quais possuem apenas um vértice. A troca de pares resulta numa nova distribuição de cores que continua a satisfazer as restrições do problema.