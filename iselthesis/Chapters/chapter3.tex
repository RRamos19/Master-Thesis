...
Na Tabela~\ref{tabela:requisitos-nao-funcionais} são apresentados os requisitos não funcionais do sistema.
...
\begin{table}[H]
    ...
    \begin{tabular}{c p{0.5\linewidth} c}
        ...
        RNF002     & Existência de feedback visual, como barras de progresso, durante a geração dos horários. & Obrigatório   \\ \midrule
        RNF003 &
...
Neste caso, cada semana tem 7 dias, o semestre letivo tem duração de 13 semanas e cada \textit{slot} tem duração de 5 minutos. Para obter a duração de cada \textit{slot} é necessário dividir o número de minutos, num único dia, pelo número de \textit{slots} totais, ou seja, $24*60/288 = 5$.
...
Na \textit{tag class} é definido o identificador da aula e a quantidade limite de estudantes que podem frequentar a aula. O limite apresentado apenas faz sentido no caso da criação dos horários ser feita após a inscrição dos estudantes, pois desta forma tem-se acesso à quantidade de estudantes que devem frequentar a disciplina em causa.
...
A duração da aula deve ser de 50 minutos e pode ser calculada da mesma forma que o início da aula, ou seja, $duração = 5*10 = 50$. É possível definir as semanas em que a aula deve ser agendada através de um formato binário. Neste exemplo, a aula deve estar agendada para todas as semanas. Por fim, é definida uma penalização caso o horário não siga a definição na sua totalidade. 
...
A definição dos estudantes consiste na atribuição de um identificador único, normalmente o número de aluno, e dos cursos ao qual o estudante pertence. Na Listagem~\ref{listagem:exemplo-estudantes} é apresentado um exemplo para demonstrar o padrão.
...
Em seguida, são apresentadas pequenas descrições das várias restrições.

\begin{compactitem}
    \item SameStart - Todas as aulas presentes na restrição devem começar à mesma hora;
    \item SameTime - Todas as aulas presentes na restrição devem ocorrer nos mesmos horários do dia exigidos pela aula mais longa do conjunto;
    \item DifferentTime - Todas as aulas presentes na restrição devem estar não sobrepostas;
    \item SameDays - Todas as aulas presentes na restrição devem ocorrer nos mesmos dias da semana. Caso uma das aulas seja lecionada num número reduzido de dias em relação às outras esta deve ser lecionada num subconjunto dos dias utilizados pela aula com maior número de dias;
    \item DifferentDays - Todas as aulas presentes na restrição devem ser lecionadas em dias da semana diferentes;
    \item SameWeeks - Todas as aulas presentes na restrição devem ser lecionadas nas mesmas semanas. Caso uma das aulas seja lecionada num número reduzido de semanas em relação às outras, esta deve ser lecionada num subconjunto das semanas utilizadas pela aula com maior número de semanas;
    \item DifferentWeeks - Todas as aulas presentes na restrição devem ser lecionadas em semanas diferentes;
    \item SameRoom - Todas as aulas presentes na restrição devem ser lecionadas na mesma sala;
    \item DifferentRoom - Todas as aulas presentes na restrição devem ser lecionadas em salas diferentes;
    ...
    \item WorkDay(S) - Previne ou penaliza quando o número de períodos entre o início da primeira aula e fim da última aula excede o número de períodos ''S'' definido na restrição;
    ...
    \item MaxDays(D) - As aulas nesta restrição não devem ser atribuídas a mais de D dias da semana independentemente de isso ocorrer durante a mesma semana de um período letivo;
    \item MaxDayLoad(S) - Limita a quantidade total de tempo atribuída ao conjunto de aulas listadas na restrição a não mais do que S intervalos de tempo por dia durante todo o semestre;
    \item MaxBreaks(R,S) - Limita o número de intervalos entre aulas durante um dia. O número de intervalos deve exceder S intervalos de tempo mas não mais do que R por dia;
    \item MaxBlock(M,S) - Limita a quantidade de tempo (medida como M slots) que um conjunto de aulas pode ser agendado consecutivamente, cada uma separada por no máximo S slots.
\end{compactitem}
...
Cada professor pode lecionar várias disciplinas e também pode ter manchas de indisponibilidade. Para identificar cada professor é utilizado um identificador, que nas instituições de ensino costuma ser um número atribuido tanto aos docentes como aos estudantes. % TODO: completar
...
Leitura de um ficheiro, organizado de acordo com o padrão do concurso \gls{itc} 2019, e armazenamento dos dados numa base de dados.
...
Após selecionado um dos horários gerados, de uma lista de horários, apresentados na interface gráfica, deve ser apresentado o horário permitindo a modificação do mesmo.
...
\begin{compactenum}
    ...
    \item Conflitos de disciplinas obrigatórias: Não haver sobreposições entre disciplinas quando, pelo menos, uma delas é obrigatória;
...
\begin{compactenum}
    ...
    \item Intervalos entre aulas: Não haver blocos ''vazios'' entre aulas.
\end{compactenum}
...
\begin{compactitem}
    \item Implementação de um sistema de importação de dados \gls{xml} de acordo com o padrão \gls{itc}~2019;
...
As tarefas que se preveem como mais simples e rápidas estão relacionadas com a implementação das bases de dados e do interpretador do padrão. Desde que o planeamento seja adequado, a implementação destas funcionalidades torna-se uma tarefa relativamente simples.

Por outro lado, as tarefas mais complexas são aquelas relacionadas com a criação da interface gráfica e a otimização do algoritmo \gls{sa}. A necessidade de que a interface gráfica seja intuitiva, eficiente e capaz de apresentar uma grande quantidade de informação com poucos cliques torna a sua criação um processo desafiador. A otimização do \gls{sa} exige uma série de testes e comparações de resultados, o que pode implicar um tempo considerável até serem encontrados parâmetros satisfatórios.

%TODO: Por terminar e melhorar