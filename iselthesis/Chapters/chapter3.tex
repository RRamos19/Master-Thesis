...
A definição dos estudantes consiste na atribuição de um identificador único, normalmente o número de aluno, e dos cursos ao qual o estudante pertence. Na Listagem~\ref{listagem:exemplo-estudantes}, é apresentado um exemplo para demonstrar a definição dos estudantes.
...
\begin{figure}[ht]
    ...
    \caption{Restrições suportadas no padrão ITC 2019 (adaptado de \cite{itc2019-Website}).}
...
Cada professor pode lecionar várias disciplinas e também pode ter manchas de indisponibilidade. Para identificar cada professor é utilizado um identificador, que nas instituições de ensino costuma ser um número atribuido tanto aos docentes como aos estudantes. A definição das manchas de indisponibilidade seguem o formato estabelecido na definição das salas.
...
As restrições flexíveis incluem redução do número de salas ocupadas e redução de intervalos entre aulas. Estas restrições pretendem aumentar o valor da solução final.
...
As tarefas previstas como mais simples e rápidas estão relacionadas com a implementação da base de dados e com o desenvolvimento do interpretador do padrão. Desde que o planeamento seja adequado, a implementação destas funcionalidades torna-se uma tarefa relativamente simples.

Por outro lado, as tarefas mais complexas estão relacionadas com o desenvolvimento da interface gráfica e com a implementação e otimização do algoritmo \gls{sa}. A necessidade de que a interface gráfica seja intuitiva, eficiente e capaz de apresentar uma grande quantidade de informação com poucos cliques torna o seu desenvolvimento um processo desafiador. A implementação do \gls{sa} exige o funcionamento do algoritmo para vários cronogramas de cursos em simultâneo, pois é necessário conciliar as restrições de cada um, de modo a ser possível gerar os horários para os diferentes cursos. A otimização do \gls{sa} requer múltiplos testes e comparações de resultados, o que implica um investimento considerável de tempo, com o objetivo de encontrar valores satisfatórios para os parâmetros do algoritmo.