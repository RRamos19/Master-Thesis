...
public class DBConnectionCache {
    ...
    private final Map<List<String>, DBManager<Collection<InMemoryRepository>>> dbManagerCacheMap = new HashMap<>();

    public DBManager<Collection<InMemoryRepository>> connectToDatabase(String ip, String port, String userName, String password) throws DatabaseException {
        List<String> key = List.of(ip, port);
        if (dbManagerCacheMap.containsKey(key)) return dbManagerCacheMap.get(key);

        DBManager<Collection<InMemoryRepository>> dbManager = new DBHibernateManager(DB_NAME, ip, port, userName, password);
